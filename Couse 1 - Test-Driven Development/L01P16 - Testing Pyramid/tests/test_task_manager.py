...
def test_integration_api_creates_task(test_app, manager):
    ...
    assert manager.get_task_by_title("Integration Test Task") is not None
...
def test_e2e_user_can_create_task_via_ui(selenium_driver):
    ...
    mock_element = selenium_driver.find_element(By.ID, "task-list")
    assert mock_element.text == "E2E Test Task Title"
